...
const API_URL = 'http://192.168.0.228:3000'; // Ajuste para o IP backendEndereço 
...
export default function Calculation() {
  ...
  const calcularMacros = async () => {
    try {
      // Exemplo: Buscar usuário cadastrado para pegar dados
      const userResponse = await axios.get(`${API_URL}/users`);
      const user = userResponse.data[0]; // pegar o primeiro usuário cadastrado, exemplo simplificado
      ...
      const res = await axios.post(`${API_URL}/calculate`, {
        ...
        objetivo: 'manutencao', // Exemplo fixo, pode ter tela para selecionar
      });
